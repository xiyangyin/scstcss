...
(function(){
	...
	function codeHint(code){
		...
		switch(code){
			...
			break;
			case 23014:
				str = '登录错误次数超限，请1小时后再试';
			break;
			case 23016:
				str = '登录错误次数超限，请24小时后再试';
			break;
			case 1015:
				str = '该账号密码已设置';
			break;
	...
	myApp.directive('passwordHint',[function(){
		return {
            ...
            template: '<div class="password-hint"><div class="bar" ng-class="{\'weaker\':pwdInfo.strong == \'weaker\',\'weak\':pwdInfo.strong == \'weak\',\'strong\':pwdInfo.strong == \'strong\'}"></div><p><b>密码强度:</b><span>{{pwdInfo.hint}}</span></p></div>',
		...
		return {
			...
			template:'\
				<div class="" style="padding-bottom: 18px;">\
					...
					<div class="voice-verifical">{{view.tips.voice}}<a class="cursor-pointer" ng-if="send" ng-click="getMobileCode($event, 1)">语音验证码</a></div>\
				</div>\
				',
			controller:['$scope','$http','$timeout',function($scope,$http,$timeout){
				...
				function modifyVal(){
					...
					function timer(){
						sum -= 1;
						$scope.view.tips.message = sum +'s后重试';
						$scope.view.tips.voice = '验证码已发送，请' + sum + 's后再获取';
						if(sum <= 0){
							$scope.send = true;
							$scope.view.tips.message = '获取验证码';
							$scope.view.tips.voice = '收不到短信？试试';
						}else{
							$timeout(function() {
								timer();
							}, 1000);
	...
	.factory('sendMailBox', ['ngDialog', function(ngDialog){
		...
		return function openMailDialog(mail,type, frequently){
            ...
            if (type == 'register') {
                dialogTitle = '邮箱注册';
                dialogContent = '已向你的邮箱 '+ mail +' 发送账号激活邮件。点击邮件中的链接即可激活该账号，设置密码后，可直接使用BDP服务。';
            } else if (type == 'login') {
                dialogTitle = '用户登录';
                dialogContent = '已向邮箱 '+ mail +' 发送找回密码邮件，请登录邮箱后点击验证链接，完成密码修改。';
            } else {
                dialogTitle = '找回密码';
                dialogContent = '已向邮箱 '+ mail +' 发送找回密码邮件，请登录邮箱后点击验证链接，完成密码修改。';
            }
            if (frequently) {
                dialogContent = "点击过于频繁，请稍候再试~";
            }
			ngDialog.open({
    ...
    myApp.directive('activationEmaile', ['$interval','$http','sendMailBox', function($interval, $http, sendMailBox){
        return {
            ...
            restrict: 'AE',
            template: '<a class="ml8 cursor-pointer" ng-click="activationEmaile()">立即激活</a>',
            replace: true,
	...
	myApp.controller('registerCtrl',['$scope','$timeout','$interval','$http','ngDialog','sendMailBox','$location',function($scope,$timeout,$interval,$http,ngDialog,sendMailBox,$location){
		...
		$scope.info = {};
		$scope.info.register_type = 2; //默认短信注册 =》 2，1=》email
		...
		*/
		function verifyPhoneReg(){
			...
			if(!view.passwordStrong){
				view.hint = '密码强度过低，请重新输入包含字母与数字的并且长度不小于8位的密码';
				return false;
			}
			return true;
		}
	...
	myApp.controller('findPwdCtrl',['$scope','$timeout','$http','sendMailBox','$interval',function($scope,$timeout,$http,sendMailBox,$interval){
		...
		$scope.info = {};
		$scope.info.verify_type = 2; //默认短信注册 =》 2，1=》email
    ...
    function infoCtrl($scope,$http){
        ...
        view.form = [
            // user_name 原来有姓名，现在没了，后端的数据库里还是有这个值的
            {
                key: "company",
                name: "公司名称",
                maxLength: 50
            },
            {
                ...
                name: "所属行业",
                maxLength: 50,
                option: ["互联网", "电商", "零售快消", "制造业", "个体", "学校/研究机构", "企事业单位", "政府部门", "咨询/公关/广告公司", "医疗/美容/保健", "金融", "房地产", "物流",  "贸易", "餐饮", "服装", "运营商", "高科技", "文化传媒", "旅游酒店", "娱乐", "其他"]
            },
            {
                key: "position",
                name: "职位",
                maxLength: 50,
                option: ["产品经理/总监", "运营经理/总监", "新媒体运营", "网络推广", "数据分析师", "数据工程师", "数据研究员", "财务会计", "项目经理", "市场", "公关", "销售", "IT", "研发", "HR", "老师", "学生", "职员", "策划", "总助", "个体（老板）", "CEO", "咨询顾问", "实施", "设计师", "其他"]
            },
            {
                key: "tool",
                name: "常用分析工具",
                maxLength: 50,
                option: ["Excel", "SPSS", "STATA", "tableau", "大数据魔镜", "Google analytics", "Python", "echarts", "友盟", "百度统计", "SQL", "R语言", "其他"]
            },
            {
                key: "demand",
                name: "注册后的预期用途",
                maxLength: 1000,
                option: ["数据可视化分析", "制作图表", "数据汇报", "数据处理", "学习/研究使用", "其他"]
            }
        ]
        ...
        $scope.submit = function () {
            if(view.send) return;
            // $scope.data.inputNames, $scope.data.info, 以前是用这俩变量做判断的，实在是不想改其他的地方了……
            data.inputNames = {};
